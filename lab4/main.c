...
int main(int argc, char **argv)
{
    ...
    gettimeofday(&tv1, &tz);
    MPI_Scatter((void *)F_matrix, n * m, MPI_DOUBLE, (void *)f_matrix, n * m, MPI_DOUBLE,
                0, MPI_COMM_WORLD);
    for (time = 0; time < time_interval / time_step; time += 1)
    {
        ...
        if (myrank != kol - 1)
            MPI_Send(f_matrix + n * (m - 1), n, MPI_DOUBLE, in1, 1,
                     MPI_COMM_WORLD);
        if (myrank)
        ...
        if (myrank != kol - 1)
            MPI_Recv(el_last, n, MPI_DOUBLE, in1, 2, MPI_COMM_WORLD,
                     status2);
        for (i = 0; i < m; i++)
            for (j = 0; j < n; j++)
            {
                ...
                { // если на границе
                    ...
                    n_matrix[i * n + j] =
                        (f_matrix[(j - 1) + n * i] - 4 * f_matrix[j + n * i] + f_matrix[(j + 1) + n * i] +
                         el_prev[j] + f_matrix[(j) + n * (i + 1)]) *
                            time_step / (R * C) +
                        f_matrix[j + n * i];
                else if (i == m - 1) // если на границе
                    n_matrix[i * n + j] =
                        (f_matrix[(j - 1) + n * i] - 4 * f_matrix[j + n * i] + f_matrix[(j + 1) + n * i] +
                         el_last[j] + f_matrix[(j) + n * (i - 1)]) *
                            time_step / (R * C) +
                        f_matrix[j + n * i];
                else // в ценрте
                    n_matrix[i * n + j] =
                        (f_matrix[(j - 1) + n * i] - 4 * f_matrix[j + n * i] + f_matrix[(j + 1) + n * i] +
                         f_matrix[(j) + n * (i + 1)] + f_matrix[(j) + n * (i - 1)]) *
                            time_step / (R * C) +
                        f_matrix[j + n * i];
            }
        MPI_Gather((void *)n_matrix, m * n, MPI_DOUBLE, (void *)N_matrix, m * n, MPI_DOUBLE,
                   0, MPI_COMM_WORLD);
        p = f_matrix;
        f_matrix = n_matrix;
        n_matrix = p;
        if (!myrank)
        { // вывод графика
            fprintf(gp, "splot [][][0:10000] '-' title 'time = %+#.3g)'\n", time);
            for (i = 0; i < num_of_elements; i++)
